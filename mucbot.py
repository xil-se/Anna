...
class MUCBot(sleekxmpp.ClientXMPP):
    ...
    def muc_command(self,msg):
        ...
        if m:
            msg['body']= m.group(2)
            if (m.group(1) in self.config['rooms'][msg['from'].bare]['enabled-handlers'] or m.group(1) in ['help']):
                self.event(m.group(1),[m.group(1),msg])
        elif m2:
            msg['body']=''
            if (m2.group(1) in self.config['rooms'][msg['from'].bare]['enabled-handlers'] or m2.group(1) in ['help']):
    ...
    def muc_message(self, msg):
        ...
        if msg['mucnick']:
            if msg['mucnick'] == 'David' and self.nick in msg['body']:
                ...
                self.send_message(mto=msg['from'].bare,
                                  mbody="%s, %s" %(msg['mucnick'],replys[self.random.randint(1,len(replys))-1] ),
                                  mtype='groupchat')
